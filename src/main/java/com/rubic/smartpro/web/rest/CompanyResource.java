...
/**
 * REST controller for managing {@link com.rubic.smartpro.domain.Company}.
 */
@RestController
@RequestMapping("/api")
public class CompanyResource {
    ...
    @PutMapping("/companies/selectedCompany")
    public String updateSelectedCompany(@RequestParam("id") String id, @RequestParam("selectedCompany") String selectedCompany) throws URISyntaxException {
        String result = companyService.updateSelectedCompany(id,selectedCompany);
        return result;
    }
    ...
    //Get Company by Selected Status

    @GetMapping("company")
    public ResponseEntity<CompanyDTO> getSelectedCompany(@RequestParam("status") String status) {
        log.debug("REST request to get Company : {}", status);
        Optional<CompanyDTO> companyDTO = companyService.findSelectedCompany(status);
        return ResponseUtil.wrapOrNotFound(companyDTO);
    }
}
