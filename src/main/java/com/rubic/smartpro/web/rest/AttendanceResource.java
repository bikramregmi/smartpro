...
/**
 * REST controller for managing {@link com.rubic.smartpro.domain.Attendance}.
 */
@RestController
@RequestMapping("/api")
public class AttendanceResource {
    ...
    public ResponseEntity<AttendanceDTO> createAttendance(@Valid @RequestBody AttendanceDTO attendanceDTO) throws URISyntaxException {
        ...
        if(attendanceService.checkAlreadyRegistered(attendanceDTO))
            throw new BadRequestAlertException("Already CheckedIn, Please Confirm Your Details", ENTITY_NAME, "idexists");
    ...
    public ResponseEntity<AttendanceDTO> updateAttendance(@Valid @RequestBody AttendanceDTO attendanceDTO) throws URISyntaxException {
        ...
        if(attendanceService.checkAlreadyRegistered(attendanceDTO))
            throw new BadRequestAlertException("Already CheckedOut, Please Confirm Your Details", ENTITY_NAME, "idexists");
        AttendanceDTO result = attendanceService.save(attendanceDTO);
    ...
    /**
     * {@code GET  /attendances} : get all the attendances for today.
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of attendances in body.
     */
    @GetMapping("/attendances/current")
    public ResponseEntity<List<AttendanceDTO>> getAllAttendancesForToday(Pageable pageable) {
        log.debug("REST request to get a page of Attendances for today");
        List<AttendanceDTO> page = attendanceService.findAllForToday(pageable);
        return ResponseEntity.ok().body(page);
    }

    @GetMapping("/attendances/current/{id}")
    public ResponseEntity<AttendanceDTO> getOneAttendancesForToday(@PathVariable Long id) {
        log.debug("REST request to get a page of Attendances for today");
        AttendanceDTO attendanceDTO = attendanceService.findOneForToday(id);
